...
impl ModuleConfig {
    ...
    pub(crate) fn read_config<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        let toml_string = std::fs::read_to_string(&config_path)
            .wrap_err_with(|| format!("Failed to read module config file: {:?}", config_path))?;
    ...
    fn expand_single_directory(
        ...
        let source_parent = conf
            ...
            .as_ref()
            .and_then(|s| s.parent())
            .ok_or_else(|| eyre!("Source {:?} has no parent directory", conf.source.as_ref()))?;
        let dest_parent = dest
            .parent()
            .ok_or_else(|| eyre!("Destination {:?} has no parent directory", dest))?;
