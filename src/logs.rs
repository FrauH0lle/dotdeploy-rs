...
impl Logger {
    ...
    pub(crate) fn start(&self) -> Result<WorkerGuard> {
        ...
        let terminal_info_layer = fmt::layer()
            ...
            .with_thread_ids(level >= Level::DEBUG)
    ...
    ///
    /// # Errors
    /// Return an error if file operations failed.
    #[instrument]
...
///
/// This builder provides a fluent interface for setting up logging configuration including
/// verbosity level, maximum number of log files to retain, and the directory where logs should be
/// stored.
...
impl LoggerBuilder {
    ...
    pub(crate) fn build(&self) -> Result<Logger> {
        Ok(Logger {
            ...
            max_logs: self.max_logs.unwrap_or(15),
...
mod tests {
    ...
    fn test_log_rotation() -> Result<()> {
        ...
        assert_eq!(remaining_logs.len(), 15, "Should keep exactly 15 log files");
        ...
        assert_eq!(
            ...
            "Last remaining file should be dotdeploy_20241028_214429.log"
        );
    ...
    fn test_get_default_log_dir() -> Result<()> {
        ...
        // Restore old value
        unsafe { std::env::set_var("XDG_DATA_HOME", old_env) };
        ...
        // Restore old values
        unsafe { std::env::set_var("XDG_DATA_HOME", old_env) };
        unsafe { std::env::set_var("HOME", old_env_home) };
