...
impl FileUtils {
    ...
    /// # Errors
    ///
    /// Returns an error if an error occurs during the checksum calculation or file reading.
    ///
    /// # Examples
    ///
    /// ```no_run
    ...
    /// }
    /// ```
    pub(crate) async fn calculate_sha256_checksum<P: AsRef<Path>>(
        &self,
        path: P,
    ) -> Result<String> {
        ...
        let checksum = match fs::read(&path).await {
            ...
            Err(e) if e.kind() == std::io::ErrorKind::PermissionDenied => {
                ...
                let output = self
                    .privilege_manager
                    .sudo_exec_output("sha256sum", [path.as_ref().to_str().unwrap()], None)
                    .await?
                    .stdout;
...
mod tests {
    ...
    async fn test_calculate_sha256_checksum() -> Result<()> {
        ...
        let checksum = fs_utils.calculate_sha256_checksum(&temp_file).await?;
        ...
        let checksum_sudo = fs_utils.calculate_sha256_checksum(&temp_file).await?;
